...
const AdminDashboard = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Top metrics — filtered by capability */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {canClients && (
          <MetricCard label="Clientes activos" value={stats?.activeClients ?? "—"} icon={Users} delta="4.9%" iconBg="bg-info/10 text-info" />
        )}
        {canTasks && (
          <MetricCard label="Tareas abiertas" value={stats?.openTasks ?? "—"} icon={KanbanSquare} delta="2.7%" iconBg="bg-primary/15 text-primary" />
        )}
        {canFinance && (
          <MetricCard label="Ingresos del mes" value={fmtMoney(stats?.monthRevenue ?? 0)} icon={DollarSign} delta="4.9%" iconBg="bg-success/10 text-success" />
        )}
        {canFinance && (
          <MetricCard label="Por cobrar" value={fmtMoney(stats?.amountDue ?? 0)} icon={Receipt} delta="3.4%" deltaPositive={false} iconBg="bg-warning/15 text-warning-foreground" />
        )}
      </div>
          ...
          <CardContent className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-0">
            {canTasks && (
              <Link to="/admin/tasks" className="rounded-xl border border-border/60 p-4 hover:border-primary/50 hover:bg-muted/40 transition-colors">
                <div className="flex items-center justify-between">
                  <div className="h-9 w-9 rounded-lg bg-destructive/10 text-destructive flex items-center justify-center"><AlertCircle className="h-4 w-4" /></div>
            ...
            {canDocs && (
              <Link to="/admin/documents" className="rounded-xl border border-border/60 p-4 hover:border-primary/50 hover:bg-muted/40 transition-colors">
                <div className="flex items-center justify-between">
                  <div className="h-9 w-9 rounded-lg bg-info/10 text-info flex items-center justify-center"><FileText className="h-4 w-4" /></div>
                  <ArrowRight className="h-4 w-4 text-muted-foreground" />
                </div>
            ...
            {canFinance && (
              <Link to="/admin/invoices" className="rounded-xl border border-border/60 p-4 hover:border-primary/50 hover:bg-muted/40 transition-colors">
                <div className="flex items-center justify-between">
                  <div className="h-9 w-9 rounded-lg bg-warning/15 text-warning-foreground flex items-center justify-center"><Receipt className="h-4 w-4" /></div>
                  <ArrowRight className="h-4 w-4 text-muted-foreground" />
                </div>
